...
```sh
tinybrains check model.onnx adapter.json       # what admission will say
...
cloned from its moves, and the export writes `model.onnx`, the `adapter.json` that describes it,
...
| `adapter.json` | The two programs the referee runs around every inference: the board in, one move per ant out. Generated, never hand-edited. |
...
| `.github/workflows/check.yml` | Runs `tinybrains check` and a match on every push, so an adapter change nobody meant is caught on the commit that made it. |

## Until a release is cut

The `tinybrains` CLI builds from source and reads the game from a checkout beside this one. Four
clones, once:

```sh
git clone https://github.com/Tiny-Brains/ants          # games.toml resolves ../ants
git clone https://github.com/Tiny-Brains/devops
git clone https://github.com/Tiny-Brains/axon          # devops/cli depends on it by path
...
2. **Change something and retrain.** `train.py` takes `--class`, `--epochs`, `--seed`; the recipe
   ...
   decides your class. A different encoding means a different adapter, and the adapter is generated
...
3. **Check it the way admission will.** `tinybrains check model.onnx adapter.json`. A pass is
   necessary and not sufficient: your machine decides no class.
4. **Publish and submit.** Commit, tag a release with `model.onnx` and `adapter.json` attached, take
   the two hashes with `shasum -a 256 model.onnx adapter.json`, and submit them on the site. The
   book's [quickstart](https://github.com/Tiny-Brains/docs/blob/main/src/quickstart.md) is the long
   form.

## What must stay true

- **`adapter.json` is generated.** Editing it by hand is how the encoding the trainer saw and the
...
- **The four files ship together.** A model and an adapter are hashed and measured as a pair, and
  the card and metrics say what that pair measured.
